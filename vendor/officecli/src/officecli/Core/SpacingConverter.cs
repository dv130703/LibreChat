...
internal static class SpacingConverter
{
    ...
    /// </summary>
    public static uint ParseWordSpacing(string value, string context = "spacing")
    {
        var points = ParseSpacingToPoints(value, bareIsPoints: false, context);
        if (points < 0)
            throw new ArgumentException($"Invalid '{context}' value '{value}'. Spacing must be non-negative.");
    ...
    /// </summary>
    public static int ParseWordSpacingSigned(string value, string context = "spacing")
    {
        var points = ParseSpacingToPointsSigned(value, bareIsPoints: false, context);
        return (int)Math.Round(points * TwipsPerPoint);
    }

    private static double ParseSpacingToPointsSigned(string value, bool bareIsPoints, string context = "spacing")
    {
        var trimmed = value.Trim();
        bool neg = trimmed.StartsWith("-");
        var rest = neg ? trimmed[1..] : trimmed;
        var pos = ParseSpacingToPoints(rest, bareIsPoints, context);
        return neg ? -pos : pos;
    }
    ...
    /// </summary>
    private static double ParseSpacingToPoints(string value, bool bareIsPoints, string context = "spacing")
    {
        var trimmed = value.Trim();

        if (trimmed.EndsWith("pt", StringComparison.OrdinalIgnoreCase))
            return ParseNumber(trimmed[..^2], context);

        if (trimmed.EndsWith("cm", StringComparison.OrdinalIgnoreCase))
            return ParseNumber(trimmed[..^2], context) * PointsPerCm;

        if (trimmed.EndsWith("in", StringComparison.OrdinalIgnoreCase))
            return ParseNumber(trimmed[..^2], context) * PointsPerInch;

        // Bare number
        var num = ParseNumber(trimmed, context);
    ...
    /// </summary>
    /// <remarks>
    /// BUG-INDENT-ERRMSG: this used to report every caller's failure as
    /// `Invalid 'spacing' value ...` with lineSpacing-only example units
    /// ('1.5x'/'150%'), regardless of which property actually failed —
    /// indent/margin callers got a message naming the wrong property AND
    /// suggesting units ('x'/'%') that indent doesn't even accept. A model
    /// given that message has no way to discover the property it actually
    /// passed was even the one that failed, let alone the units it supports.
    /// Now the caller's own property name is threaded through as `context`
    /// (see ParseWordSpacingSigned's `context` parameter), and the example
    /// units shown match what that property family actually parses.
    /// </remarks>
    private static double ParseNumberAllowNegative(string s, string context)
    {
        ...
        {
            var examples = context == "lineSpacing"
                ? "'12pt', '1.5x', '150%'"
                : "'12pt', '0.5cm', '0.5in'";
            throw new ArgumentException(
                $"Invalid '{context}' value '{s}'. Expected a finite number with optional unit (e.g. {examples}).");
        }
        return result;
    }
}
