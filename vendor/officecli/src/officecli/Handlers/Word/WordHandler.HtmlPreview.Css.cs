...
public partial class WordHandler
{
    ...
    /// still missing. Terminal goal is full-fidelity built-in style table.
    /// Internal (not private) so <see cref="OfficeCli.BlankDocCreator"/> can
    /// reuse the exact same numbers to actually define these styles in a new
    /// document's styles.xml — see BUG-STYLE-UNDEFINED below.</summary>
    internal record BuiltInStyleDefault(
        double Before, double After, double Line,
        double? SizePt, bool Bold, string? ColorHex);

    internal static readonly System.Collections.Generic.Dictionary<string, BuiltInStyleDefault> BuiltInStyleDefaults
