...
public partial class WordHandler
{
    ...
    private static readonly Regex OrderedMarkerRegex = new(@"^\s*\d+[.)]\s+(.*)$", RegexOptions.Compiled);

    /// <summary>See AUTO-LIST-MARKER below: strips a leading "- "/"* "/"N. " marker
    /// from paragraph text and reports which listStyle it implies, or
    /// (text, null) unchanged if no marker is present.</summary>
    private static (string text, string? listStyle) StripLeadingListMarker(string text)
    {
        var bulletMatch = BulletMarkerRegex.Match(text);
        if (bulletMatch.Success)
            return (bulletMatch.Groups[1].Value, "bullet");
        var orderedMatch = OrderedMarkerRegex.Match(text);
        if (orderedMatch.Success)
            return (orderedMatch.Groups[1].Value, "ordered");
        return (text, null);
    }
    private string AddParagraph(OpenXmlElement parent, string parentPath, int? index, Dictionary<string, string> properties)
    {
        ...
        if (properties.TryGetValue("text", out var rawParaText) && rawParaText != null)
        {
            ...
            if (normalized.IndexOf('\n') >= 0)
            {
                ...
                string firstPath = "";
                // AUTO-LIST-MARKER leak guard: the recursive AddParagraph call
                // below mutates this SAME shared `properties` dict in place —
                // including injecting "listStyle" when it auto-detects a "- "/
                // "1. " marker on a line that doesn't have one explicitly set.
                // Without resetting it every iteration, line 1's auto-detected
                // "listStyle" would still be sitting in the dict when line 2 is
                // processed, and line 2's own marker-detection would then skip
                // itself (it only runs when listStyle is ABSENT) — stripping
                // the marker on the first line only and leaving every
                // subsequent line's literal "- " untouched.
                var hadListStyleBefore = properties.ContainsKey("listStyle") || properties.ContainsKey("liststyle");
                try
                {
                    for (int li = 0; li < lines.Length; li++)
                    {
                        ...
                        if (li == 0) firstPath = p;
                        if (!hadListStyleBefore)
                        {
                            properties.Remove("listStyle");
                            properties.Remove("liststyle");
                        }
                    }
                }
        ...
        // AUTO-LIST-MARKER: add_element's own tool description explicitly
        // tells a calling model to use the listStyle property instead of
        // typing a literal "-"/"1." prefix — but a live trace (2026-09-05)
        // showed a model doing exactly that anyway, out of habit, even with
        // the instruction right in front of it. Rather than silently
        // producing a paragraph that LOOKS like a list item but isn't one
        // (no real numbering, doesn't renumber, ignored by outline/TOC),
        // detect the same leading markers `add type="markdown"` already
        // recognizes and convert them into real listStyle formatting. Only
        // when the caller didn't already set listStyle themselves — an
        // explicit choice always wins over this fallback.
        if (!properties.ContainsKey("listStyle") && !properties.ContainsKey("liststyle")
            && properties.TryGetValue("text", out var markerText) && markerText != null)
        {
            var (strippedText, detectedListStyle) = StripLeadingListMarker(markerText);
            if (detectedListStyle != null)
            {
                properties["text"] = strippedText;
                properties["listStyle"] = detectedListStyle;
            }
        }
            ...
            if (numIdVal > 0)
            {
                ...
                if (!numExists)
                    throw new ArgumentException(
                        ...
                        "For a normal bulleted/numbered list, set the paragraph's \"listStyle\" property to " +
                        "\"bullet\" or \"ordered\" instead — it creates and reuses the underlying numbering " +
                        "definition automatically, no manual numId needed. Only use a raw numId if you already " +
                        "created one yourself (add /numbering --type num), or use numId=0 to remove numbering.");
            }
