...
public partial class WordHandler
{
    ...
    public List<DocumentIssue> ViewAsIssues(string? issueType = null, int? limit = null)
    {
        ...
        // and Hangul Syllables — the scripts that actually use a
        // character-counted first-line indent as a manuscript convention.
        // Deliberately text-content-based rather than locale/theme-font-based:
        // a document's declared locale can be wrong or absent, but a
        // paragraph that is actually CJK text always contains these code
        // points.
        static bool ContainsCjkText(string? text)
        {
        ...
        foreach (var para in GetBodyElements(body).OfType<Paragraph>())
        {
            ...
            if (pProps != null && IsNormalStyle(styleName))
            {
                ...
                if (!hasFirstLineChars && (indent?.FirstLine == null || indent.FirstLine.Value == "0"))
                {
                    ...
                    // BUG-INDENT-LOCALE: 2-character first-line indent is a CJK
                    // manuscript convention (matches Word's own CJK templates) —
                    // it is NOT how English/Latin-script body text is
                    // conventionally formatted (block paragraphs, no first-line
                    // indent, separated by paragraph spacing instead). This rule
                    // used to fire on ANY "Normal"-style paragraph regardless of
                    // language, so a plain English memo got flagged for missing a
                    // convention it was never supposed to follow — burning a
                    // model's tool budget "fixing" something that wasn't broken.
                    // Only flag it when the paragraph's own text is actually CJK.
                    if (!hasHanging && !hasHangingChars && !isCentered && !isList
                        && runs.Any(r => !string.IsNullOrWhiteSpace(GetRunText(r)))
                        && ContainsCjkText(GetParagraphText(para)))
                    {
                        issues.Add(new DocumentIssue
                        {
                            Id = $"F{++issueNum}",
                            ...
                            Suggestion = "Set firstLineChars to 2 (set_properties property \"firstLineChars\", a plain integer — not \"firstLineIndent\", which takes points/cm/in instead of a character count)."
                        });
                    }
                }
            }
