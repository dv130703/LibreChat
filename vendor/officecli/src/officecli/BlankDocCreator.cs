...
public static class BlankDocCreator
{
    ...
    private static void CreateWord(string path, string? locale = null, bool minimal = false)
    {
        ...
        else
        {
            ...
            // BUG-STYLE-UNDEFINED: `add_element`/`set_properties` accept
            // properties={"style": "Heading1"} (the documented, only way this
            // engine exposes to create a heading — see
            // Mcp/Tools/AddElementTool.cs) and happily write <w:pStyle
            // w:val="Heading1"/> onto the paragraph — but until now a blank
            // doc's styles.xml only ever defined "Normal". A style id that is
            // referenced but never defined renders as plain body text in
            // every consumer (real Word included — there is no template here
            // for it to inherit Heading1's bold/size/spacing from), so every
            // "heading" came out visually identical to a normal paragraph.
            // Bake in real definitions for the standard built-ins up front,
            // using the exact same numbers WordHandler's HTML preview already
            // uses to fake this visually — so the preview and the real
            // OOXML output finally agree instead of only the preview looking
            // right.
            foreach (var style in BuiltStandardStyles())
                styles.Append(style);
    ...
    /// <summary>Real Word display names for the built-in style ids, per Word's
    /// own styles.xml convention (lowercase + space for headings). Must match
    /// `builtInNamesForUpsert` in WordHandler.Add.Structure.cs — that set is
    /// what lets a later explicit `add style` call upsert one of these
    /// instead of erroring as "already exists".</summary>
    private static readonly System.Collections.Generic.Dictionary<string, string> StandardStyleDisplayNames
    ...
    /// <summary>
    /// Builds real &lt;w:style&gt; definitions for every standard built-in
    /// paragraph style (Heading1-9, Title, Subtitle, Quote, IntenseQuote,
    /// ListParagraph) from the same size/bold/color/spacing numbers
    /// WordHandler's HTML preview uses to fake these visually when a style is
    /// referenced but undefined (see BuiltInStyleDefaults in
    /// WordHandler.HtmlPreview.Css.cs). "Normal" is excluded — CreateWord
    /// already defines it separately as the doc's default style.
    /// </summary>
    private static System.Collections.Generic.IEnumerable<Style> BuiltStandardStyles()
    {
        foreach (var (styleId, def) in OfficeCli.Handlers.WordHandler.BuiltInStyleDefaults)
        {
            if (string.Equals(styleId, "Normal", StringComparison.OrdinalIgnoreCase)) continue;
            if (!StandardStyleDisplayNames.TryGetValue(styleId, out var displayName)) continue;
            yield return BuildStandardStyle(styleId, displayName, def);
        }
    }

    private static Style BuildStandardStyle(string styleId, string displayName, OfficeCli.Handlers.WordHandler.BuiltInStyleDefault def)
    {
        var pPr = new StyleParagraphProperties();
        if (def.Before > 0)
            (pPr.SpacingBetweenLines ??= new SpacingBetweenLines()).Before = ((int)Math.Round(def.Before * 20)).ToString();
        if (def.After > 0)
            (pPr.SpacingBetweenLines ??= new SpacingBetweenLines()).After = ((int)Math.Round(def.After * 20)).ToString();
        if (def.Line > 0)
        {
            var spacing = pPr.SpacingBetweenLines ??= new SpacingBetweenLines();
            spacing.Line = ((int)Math.Round(def.Line * 240)).ToString();
            spacing.LineRule = LineSpacingRuleValues.Auto;
        }
        var keepNext = styleId.StartsWith("Heading", StringComparison.OrdinalIgnoreCase)
        ...
        if (keepNext) pPr.KeepNext = new KeepNext();

        var rPr = new StyleRunProperties();
        var hasRPr = false;
        if (def.Bold) { rPr.Bold = new DocumentFormat.OpenXml.Wordprocessing.Bold(); rPr.BoldComplexScript = new BoldComplexScript(); hasRPr = true; }
        if (def.SizePt.HasValue)
        {
            var halfPoints = ((int)Math.Round(def.SizePt.Value * 2)).ToString();
            rPr.FontSize = new DocumentFormat.OpenXml.Wordprocessing.FontSize { Val = halfPoints };
            rPr.FontSizeComplexScript = new FontSizeComplexScript { Val = halfPoints };
            hasRPr = true;
        }
        if (!string.IsNullOrEmpty(def.ColorHex))
        {
            rPr.Color = new DocumentFormat.OpenXml.Wordprocessing.Color { Val = def.ColorHex.TrimStart('#') };
            hasRPr = true;
        }

        // Deliberately no <w:link> to a "<StyleId>Char" character style — this
        // engine doesn't define paired character styles, and a link to an
        // undefined style id is exactly the BUG-STYLE-UNDEFINED class of bug
        // this method exists to fix. Next always falls back to Normal: none
        // of these are styles a document should keep typing in by default.
        var style = new Style(
