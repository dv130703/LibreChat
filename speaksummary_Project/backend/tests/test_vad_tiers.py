...
it into per-word verdicts. No model involved: these are the decisions about what
counts as borderline, and they have to be right independently of which VAD
produced the timelines."""

import numpy as np
import pytest

from app.config import Settings
from app.services.vad import PyannoteVad
from app.services.vad_tiers import (
    SAMPLE_RATE,
    VadTiers,
    compute_tiers,
    contains,
...
def test_words_are_tagged_by_the_tier_their_timing_falls_in():
    ...
    assert [word["vad_confidence"] for word in segments[0]["words"]] == ["high", "borderline"]
...
def test_a_segment_spanning_both_tiers_is_flagged():
    ...
    assert segments[0]["vad_borderline"]
...
def test_a_wholly_confident_segment_is_not_flagged():
    ...
    assert not segments[0]["vad_borderline"]
...
def test_unalignable_words_are_left_untagged_rather_than_guessed():
    ...
    assert "vad_confidence" not in segments[0]["words"][0]
    assert not segments[0]["vad_borderline"]
...
def test_lowering_the_thresholds_never_loses_speech(vad):
    ...
    settings = Settings(vad_borderline_onset=0.150, vad_borderline_offset=0.100)
    ...
    assert not subtract_intervals(tiers.borderline, subtract_intervals(tiers.union, tiers.confident))
...
def test_matching_tiers_leave_nothing_borderline(vad):
    ...
    audio = _speech_like_audio()
    settings = Settings(vad_borderline_onset=0.500, vad_borderline_offset=0.363)
