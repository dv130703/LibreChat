...
**10. Fix #2's new-conversation path (auto-navigate straight to `?panel=transcript`) was itself the wrong design, not just buggy** - reported directly by the user after #6 landed: submitting audio produces an extremely long wait with zero acknowledgment in the chat window, and once transcription finishes nothing shows there either. Two compounding problems: (a) navigating away *immediately* upon queuing (not once transcription finishes - it can't wait that long) discards whatever the user had already typed in the composer, since that draft's state belongs to the component instance being left behind; (b) even setting that aside, jumping straight to the side panel with **no message ever sent** means the main chat window - what the user is actually looking at - stays empty the entire time, and the transcript panel is the *only* place anything is visible, which is not where anyone is looking during a 1-2 minute cold model load. Auto-sending a message immediately isn't a fix either: the model would answer *before* the transcript (and its RAG embedding) exists, reproducing the exact "I don't see any text to summarize" failure from earlier in this log - the job's own logs show a cold start alone routinely taking 90+ seconds before transcription even starts, let alone finishes and embeds. Fixed by making the new-conversation path behave identically to the existing-conversation path (§6.4): the file is added to the composer as a normal pending attachment (`filesByIndex`, keyed by pane index rather than conversation id, so it survives the navigate unaffected) and the URL navigate drops `?panel=transcript` entirely - there is no message yet for a panel to be "the transcript for." The user sees the attachment chip the instant they land on the new conversation, types their question, and sends when *they* choose to (by which point, if they waited for the card to show "ready," the transcript actually exists for the model to read) - the `TranscriptCard`'s own queued/transcribing/ready states are what make that wait visible in the interim, and its "Open transcript" link is what makes the panel reachable, exactly as it already worked for an existing conversation.

**11. Fix #10 reported by the user as making things worse, not better - "blank canvas with no side panel nor the audio above."** After #10 shipped (composer chip only, `?panel=transcript` dropped), the user reported landing on the new conversation to an empty pane: neither the file chip nor anything else was visible. Root-caused only as far as code-reading could establish: `filesByIndex` is keyed by pane *index* (`store/families.ts`), so in principle the chip write from before `navigate()` should survive the switch, but `useNewConvo.ts`'s `newConversation()`/`switchToConversation()` hydration path (`useGetEndpointsQuery`, `useGetModelsQuery`, `useApplyModelSpecEffects`, several Recoil resets) is complex enough that a real conflicting reset somewhere in that path was plausible, and code-reading alone could not confirm or rule it out. Rather than ship another unverified guess, `?panel=transcript` was restored alongside the still-present composer-chip write, as a pragmatic hedge: the panel-based navigation is the one path in this whole log with independent, confirmed-working coverage (fix #6, `ChatPanelHost`'s URL-param handling), so restoring it should at minimum bring back a visible acknowledgment the instant the user lands on the new conversation, even though it reintroduces the two problems #10 was written to solve (draft text discarded, main chat window empty behind the panel). This was communicated to the user explicitly as a hedge, not a confirmed fix - the deeper question of whether `ChatView`/`ChatForm`'s hydration reliably renders `filesByIndex` state for a conversation reached via this specific `navigate()` call remains open, and should be closed out with either a real dev-server run or a `ChatRoute`-level integration test, matching the verification bar every other fix in this log met.

**12. Root cause of #11, finally identified: there was never a message for the panel or the composer chip to attach to.** The user clarified what "acknowledge the upload" actually meant: not an auto-opened panel, and not a composer draft that only becomes visible once *they* send something - a real message bubble the instant upload finishes, exactly like every other chat attachment, that can be clicked to reopen its transcript - and multiple recordings in one conversation should each keep their own revisitable card. That's precisely what `Files.tsx`'s `isAudioOrVideo` branch and `TranscriptCard` already render (Phase 4, §6) - the gap was that nothing in the composer-intercept path (`POST /api/transcribe`) ever created a `message.files`-bearing message; it only ever called `db.addConvoFile`, leaving the composer-chip/`?panel=transcript` hacks in #10/#11 as the only "something happened" signals, both fighting the same underlying absence rather than fixing it. **Fixed by having the backend create the message itself**, synchronously, in the same request that queues the job: `POST /api/transcribe` now also calls `db.saveMessage` with `isCreatedByUser: true`, `files: [sourceFile]`, and `parentMessageId` (either the conversation's current leaf, now passed by the client via `useChatContext().latestMessageId`, or `Constants.NO_PARENT` for a brand-new conversation) - then `db.saveConvo(reqCtx, { conversationId }, ...)` to refresh the conversation's `messages` pointer. This is deliberately the same non-streaming `saveMessage`-outside-the-ask-pipeline pattern `POST /api/messages/:conversationId` already uses (`api/server/routes/messages.js:311-333`), not a new primitive - no model is invoked, so this doesn't reproduce the "no transcript to read yet" failure auto-sending a message was rejected for in #10; it's a `text: ''` message that exists purely to carry the attachment. One real bug caught by the new backend test (`transcribe.spec.js`, "against an existing conversation" describe block) before it shipped: calling `saveConvo(reqCtx, savedMessage, ...)` - the `messages.js` precedent's exact call shape - throws a Mongoose CastError, because `savedMessage.files` (an array of file *objects*) collides with `Conversation.files` (a plain `[String]` of ids) once spread into the same update; `saveConvo` swallows the error internally rather than throwing, so this would have shipped as a silently-stale `conversation.messages` pointer, not a visible failure. Fixed by passing only `{ conversationId }` - `saveConvo` unconditionally re-derives `messages` from what's actually persisted, so the id alone is sufficient. On the frontend, this let `useFileHandling.ts` drop both hedges from #10/#11 outright: no more `addFile`-as-composer-chip call, no more `?panel=transcript` on the navigate - just `queryClient.invalidateQueries([QueryKeys.messages, conversationId])` so an existing conversation's view picks up the new message immediately, and a plain `navigate(`/c/${conversationId}`)` for a new one, relying on the same `useGetMessagesByConvoId` query every other conversation already renders through reliably instead of either fragile, independently-hydrating path. Verified with new tests at both layers: `transcribe.spec.js` (message rooted at `NO_PARENT` for a new conversation; message linked via `parentMessageId` onto an existing conversation's current leaf, confirmed via the seeded leaf message's id) and `useFileHandling.test.ts` (formData carries the right `parentMessageId` for both cases; no more `addFile` call for the transcribed file; `invalidateQueries` called with the right messages key).

**13. The audio player itself could vanish permanently, and the desktop layout had no way to close the panel at all.** Two separate reports, addressed together since both are about the panel's own header chrome. (a) `TranscriptHeader`'s `<audio src>` came from `useFileDownload` - fetch the *entire* file into a JS `Blob` via axios, `URL.createObjectURL` it once, and cache that single blob URL with `staleTime`/`cacheTime: Infinity` and `retry: false`. Any single failure of that one fetch (a network blip, a backgrounded tab, a slow proxied download of a large recording) left `audioQuery.data` `undefined` forever - the mount effect that triggered `.refetch()` only re-ran on `[sourceFileId, user?.id]` changing, never on the data itself going stale, and `TranscriptHeader` unconditionally `return null`ed for any missing `audioSrc`, with no distinction between "still loading" and "failed for good." The whole player - not just a spinner, the entire waveform/transport - disappeared with no trace it was ever supposed to be there, exactly as reported ("it just disappears forever and there is no way to get it back"). (b) Separately, the user asked for the audio to "point to the location within the filesystem" instead of this caching scheme, and for an 'X' to close the panel - the latter turned out to already half-exist: `ChatPanelHost.tsx` had a working `closePanel` callback, but it was only ever wired to the mobile full-screen overlay's own wrapper button; the desktop resizable-split-pane layout never passed it to the panel at all, so a desktop user had no way to close the transcript panel short of editing the URL - not fixable by "look harder," the prop genuinely didn't exist on that path.

Fixed both by replacing the blob-download design entirely rather than patching around its retry logic: `GET /:sourceFileId/audio-token` (new, in `transcribe.js`, behind the normal `requireJwtAuth` chain, ownership-checked via the existing `assertOwnsSourceFile`) mints a 6-hour single-purpose JWT; the transcript panel now sets `<audio src>` directly to `transcribeStream.js`'s new `GET /:sourceFileId/audio?token=...` - a *separate* router, deliberately mounted at the same `/api/transcribe` prefix but registered *before* `routes.transcribe` in `api/server/index.js` so this one path is matched ahead of `router.use(requireJwtAuth)`, since a native `<audio>` element's own `src` request can't carry an `Authorization` header. That route verifies the token itself, re-checks ownership from scratch (so the token grants nothing beyond what `requireJwtAuth` would have anyway), and streams bytes straight from wherever they're actually stored - for local storage (`getLocalFileStream`, extended with a new optional `range` param, `fs.createReadStream(fullPath, range)`) with real `Range`/`206` support, so scrubbing a 22-minute recording seeks instead of re-downloading from byte 0. The browser's own media/network stack now owns retry and buffering; `useTranscribeAudioTokenQuery` (the client hook minting the token) uses react-query's ordinary retry/backoff instead of `retry: false`, because the only thing that request can fail at is minting a token - no file bytes move through it - so a transient failure recovers on its own instead of wedging permanently. `TranscriptHeader` also gained a real distinction it never had before: `hasError` (true only once that retry budget is exhausted) renders a visible "couldn't load the audio - retry" row instead of silently returning `null`, so even a hard failure is now recoverable by hand, not just automatically.

The close button: `PanelComponentProps` gained `onClose` (`ChatPanelHost`'s existing `closePanel`, now threaded to `PanelComponent` for both the desktop and mobile layouts rather than only the mobile overlay's own wrapper). `TranscriptPanelHeader` gained a `showActions` flag and an always-rendered close button - the roster/retranscribe/export actions still only make sense once a transcript exists (`showActions={!isLoading && lines.length > 0}`), but the header itself, title, and close button now render in every state (loading/queued/transcribing/failed/error/ready), so the panel is closable no matter what it's currently showing, not just once fully loaded.

Verified with new tests, all against real behavior rather than mocks where the fix's own correctness was actually at stake: `transcribeStream.spec.js` (new file - real Express app, real local-disk `fs.createReadStream`, no `requireJwtAuth` mounted at all, matching the real deployment exactly) covers a valid token streaming the full file with correct headers, 401 for a missing/forged token, 404 for a token whose userId doesn't own the file, a satisfiable `Range` request returning exactly the right byte slice as `206`, and an unsatisfiable range falling back to a full `200` rather than erroring. `transcribe.spec.js` gained coverage for `/audio-token` minting a token that actually `jwt.verify`s to the caller's id and 404ing for a file the caller doesn't own. `TranscriptHeader.spec.tsx` (new file) covers the three real render states - still-loading (`null`, unchanged), definitively failed (visible retry row, `onRetry` wired), and normal playback (`<audio src>` set correctly). `ChatPanelHost.spec.tsx` gained a test proving the desktop layout - previously with no close mechanism at all - now hands the panel a working `onClose`. Not covered by a new test: `TranscriptPanel.tsx`'s own integration of the token query end-to-end (the component has no existing test harness at all - Phase 4's only coverage of it is `ChatPanelHost.spec.tsx`'s stub - and building one from scratch was out of scope for this pass); this is a real gap relative to this log's usual verification bar, flagged rather than glossed over.
